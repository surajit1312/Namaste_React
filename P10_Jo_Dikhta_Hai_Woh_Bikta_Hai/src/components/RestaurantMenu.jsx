...
const RestaurantMenu = () => {
    ...
    return (
        <div>
            <div className='flex flex-wrap justify-around bg-pink-50'>
                <div>
                    <h3>{name}</h3>
            ...
            <div className='mx-60 my-8'>
                <h2 className='mx-3 my-20 font-extrabold'>Menu</h2>
                <ul className='list-none'>
                    {
                        resInfoData.map(item => {
                            if (item?.card?.info) {
                                const menuInfo = item?.card?.info;
                                return (<li className='mx-3 my-8 p-3'>{menuInfo?.name} - ₹{(menuInfo?.price / 100) || menuInfo?.defaultPrice / 100}</li>);
                            }
                        })
                    }
                </ul>
